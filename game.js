...
var pushFunction = function() { //push items into empty arrays
    for (var i = 0; i < possibleCoins; i++) {
        coins.push(new Coin(random(50, 350),(i * -225)));
        extraLives.push(new Lives(random(50, 350),(-800 - 500 * i * i)));//by using i*i we make the hearts spread further apart, so you get less extra lives later on in the game making it harder
        barrels.push(new Barrel(random(50, 350),(i * -300)));
        speed.push(1);
    }
};

pushFunction(); //push items into array
...
PlayerCharacter.prototype.checkForCoinGrab = function (coins) {
    if (coins.x > this.x - 11 && coins.x < this.x + 34 && coins.y < this.y + 50 && coins.y > this. y - 20) {
        score += 5;
        coins.x = -300;
...
PlayerCharacter.prototype.checkForBarrelGrab = function (barrels) {  
    if (barrels.x <= (this.x + 20) && barrels.x >= (this.x - 70) && barrels.y <= (this.y + 50) && barrels.y >= (this.y - 50)) {
        score -= 10;
        currentLives -= 1;
        barrels.x = 600;
